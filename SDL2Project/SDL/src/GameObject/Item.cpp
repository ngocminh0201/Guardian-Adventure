...
void Item::Update(SDL_Rect character, GameMap* MAP)
{
    if (!fell)
    {
        velY += gravity;
...
void Item::dropItem(std::vector<Item>& vItem, int numCoin, int numGem, int char_hp) const
{
    for (int i = 0; i < min(numCoin, 10); i++)
    {
        Item nItem;
    ...
    if (t == 1)
    {
        ...
        nItem.setVal(max(1, char_hp / 5));
        vItem.push_back(nItem);
    }
}

void Item::collisionX(GameMap* MAP)
{
...
void Item::collisionY(GameMap* MAP)
{
    bool ok = 0;
