...
void BossLevel::Update(Character* character, std::vector<Item>& vItem)
{
    ...
    if (Boss.getHp() == 0)
    {
        ...
        if (status == STATUS::DIE && frame == 210)
            character->setStatus(Character_STATUS::VICTORY);
...
void BossLevel::attack1(SDL_Rect character)
{
    ...
    if (timer1 < 5)
    {
        ...
        Boss.setY(min(Boss.getY(), SCREEN_HEIGHT - TILE_SIZE - Boss.getH()));
        if (frame == 17)
        {
...
void BossLevel::showBossHealthbar(SDL_Renderer* renderer)
{
    ...
    /*sf = TTF_RenderText_Solid(font, (int2str(Boss.getHp()) + "/" + int2str(Boss.getMaxHp())).c_str(), white);
    tt = SDL_CreateTextureFromSurface(renderer, sf);
    w = sf->w; h = sf->h;
    nRect.x = healthbar.getX() + (healthbar.getW() - w) / 2;
    nRect.y = healthbar.getY() + (healthbar.getH() - h) / 2 + 50;
    nRect.w = w, nRect.h = h;
    SDL_RenderCopy(renderer, tt, NULL, &nRect);
    SDL_DestroyTexture(tt); tt = NULL;
    SDL_FreeSurface(sf); sf = NULL;*/
