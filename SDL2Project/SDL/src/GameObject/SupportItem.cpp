...
void SupportItem::loadVar(pii _idle_, pii _move_, pii _victory_)
{
    _idle = _idle_;
    _move = _move_;
    _victory = _victory_;
...
void SupportItem::update(SDL_Rect character, GameMap* MAP, std::vector<Item>& vItem)
{
    rect.x += velX;
    ...
    else
    {
        if (rect.x <= SCREEN_WIDTH / 2 && dropped == false)
        {
            Item nItem;
            nItem.setId(3);
            nItem.setX(max(character.x, SCREEN_WIDTH / 2));
            nItem.setY(0);
            nItem.setW(40);
            nItem.setH(40);
...
void SupportItem::show(SDL_Renderer* renderer, int view, SDL_Texture* texture)
{
...
void SupportItem::collisionY(GameMap* MAP)
{
